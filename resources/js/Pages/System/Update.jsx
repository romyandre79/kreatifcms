                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button
                                ...
                                className={`px-8 py-3 font-bold rounded-2xl shadow-xl transition-all flex items-center gap-2 ${updateInfo.is_up_to_date
                                ...
                                {log.map((entry, idx) => (
                                    <div key={idx} className="group animate-in fade-in slide-in-from-left-4 duration-300">
                                        <div className="flex items-center gap-3 mb-1">
                                            <span className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter ${entry.status === 'success' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                                                }`}>
                                                {entry.step}
                                            </span>
                                            <span className="text-gray-500">{entry.command}</span>
                                        </div>
                                        <pre className={`whitespace-pre-wrap pl-6 border-l border-gray-800 ml-2 ${entry.status === 'success' ? 'text-gray-400' : 'text-red-400'
                                            }`}>
                                            {entry.output}
                                        </pre>
                                    </div>
